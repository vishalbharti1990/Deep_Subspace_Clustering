...
class ODSC(object):
    def __init__(self, n_input, kernel_size, n_hidden, reg_const1 = 1.0, reg_const2 = 1.0, learning_rate = 0.001,batch_size = 200, reg = None, \
        ...
        tf.compat.v1.summary.scalar("reg_loss", reg_const1 * self.reg_losses )
        
        self.selfexpress_losses = 0.5 * tf.reduce_sum(input_tensor=tf.pow(tf.subtract(z_c, z), 2.0))
        
        tf.compat.v1.summary.scalar("selfexpress_loss", reg_const2 * self.selfexpress_losses )
    ...
    def encoder(self,x, weights):
        ...
        olayer1 = tf.add(tf.nn.conv2d_transpose(x, weights['oenc_w0'],tf.stack([tf.shape(input=self.x)[0],shapes_en[1]*2,56,self.n_hidden[0]]),\
            strides=[1,2,2,1],padding='SAME'),weights['oenc_b0'])
        olayer1 = tf.nn.relu(olayer1)
        # shapes.append(layer1.get_shape().as_list())
        olayer2 = tf.add(tf.nn.conv2d_transpose(olayer1, weights['oenc_w1'],tf.stack([tf.shape(input=self.x)[0],shapes_en[1]*4,shapes_en[2]*4,self.n_hidden[2]]),\
...
if __name__ == '__main__':
    ...
    data = sio.loadmat(str(curr_dir.joinpath('../Data/mnist-original.mat')))
    ...
    model_path = str(curr_dir.joinpath('mymodels/mnist/modelovern')) 
